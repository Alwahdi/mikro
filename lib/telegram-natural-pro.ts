...
import { handleTelegramDiagnose } from "./telegram-diagnose";
...
async function saveContext(uid:number,p:ProResult){let type:string|null=null,value:string|null=null;if(p.entities.card){type="card";value=p.entities.card;}else if(p.entities.vlan_id){type="vlan";value=String(p.entities.vlan_id);}else if(p.entities.network_index){type="network";value=String(p.entities.network_index);}await dbUpsert("tg_nlu_context",{telegram_user_id:uid,last_intent:p.intent,last_entity_type:type,last_entity_value:value,updated_at:new Date().toISOString()},"telegram_user_id");}
...
export async function handleTelegramNaturalPro(update:TgUpdate):Promise<boolean>{
  ...
  try{
    ...
    if(p.intent==="diagnose"){await log(update,p,"routed:diagnose-v2");return handleTelegramDiagnose(update);}
