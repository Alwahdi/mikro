...
export async function handleTelegramCommandRouter(update:TgUpdate):Promise<boolean>{
  ...
  const aliases:Record<string,string>={"/health":"/status","/network":"/status","/active":"/online","/clients":"/online","/sale":"/sales","/today":"/sales","/vlanlist":"/vlans","/vlanslist":"/vlans","/device":"/router","/version":"/router","/test":"/ping","/internet":"/ping","/errors":"/logs","/leases":"/dhcp","/usage":"/top","/jobs":"/schedules","/finduser":"/users","/searchuser":"/users","/userfind":"/users","/notifications":"/alerts","/notify":"/alerts"};
  if(aliases[command])return handleTelegramCommandRouter(withText(update,aliases[command]+(rest?` ${rest}`:"")));
  if(command==="/diagnose"||command==="/check")return handleTelegramDiagnose(update);
