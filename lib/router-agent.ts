...
export function newAgentSecret() { return randomBytes(24).toString("base64url"); }
export function encryptAgentSecret(secret: string) { return encryptSecret(secret); }
...
export async function authenticateAgent(networkId: string, token: string) {
  const rows = await dbGet<AgentNetwork>("tg_networks", { id: `eq.${networkId}`, connection_mode: "eq.agent", select: "id,telegram_user_id,label,connection_mode,agent_secret_ciphertext,agent_last_seen_at,identity,router_os_version", limit: "1" });
  const network = rows[0]; if (!network?.agent_secret_ciphertext) return null;
  let expected = ""; try { expected = decryptSecret(network.agent_secret_ciphertext); } catch { return null; }
  if (!safeEqual(expected, token)) return null; return network;
}
