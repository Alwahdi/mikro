...
export async function authenticateAgent(networkId: string, token: string) {
  const rows = await dbGet<AgentNetwork>("tg_networks", {
    ...
    select:
      "id,telegram_user_id,label,connection_mode,agent_secret_ciphertext,agent_last_seen_at,identity,router_os_version",
    limit: "1",
  });
  ...
  let expected = "";
  try {
    expected = decryptSecret(network.agent_secret_ciphertext);
  } catch {
    return null;
  }
