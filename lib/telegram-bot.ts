...
type BotUser = { telegram_user_id: number; setup_state: string; setup_data: Record<string, unknown>; active_network_id?: string | null };
type Network = {
  id: string;
...
type UMCard = { username: string; first_profile: string | null; first_vlan_id: number | null; first_seen_router_time: string | null };
...
const mainKeyboard = {
  inline_keyboard: [
    [{ text: "➕ إضافة شبكة", callback_data: "add_network" }, { text: "📡 شبكاتي", callback_data: "networks" }],
    [{ text: "📊 الحالة", callback_data: "status" }, { text: "💰 مبيعات اليوم", callback_data: "sales" }],
    [{ text: "🧩 VLAN", callback_data: "vlan_help" }, { text: "🌐 Ping", callback_data: "ping" }],
    [{ text: "⚙️ إعدادات", callback_data: "settings" }, { text: "❓ مساعدة", callback_data: "help" }],
  ],
};

const protocolKeyboard = {
  inline_keyboard: [
    [{ text: "🔐 API-SSL (8729)", callback_data: "proto_api_ssl" }],
    [{ text: "🔌 API (8728)", callback_data: "proto_api" }],
...
async function ensureUser(user: TgUser): Promise<BotUser> {
  ...
  const rows = await dbGet<BotUser>("tg_users", { telegram_user_id: `eq.${user.id}`, select: "*" });
  return rows[0];
}

function setState(userId: number, setup_state: string, setup_data: Record<string, unknown> = {}) {
  return dbPatch("tg_users", { setup_state, setup_data, updated_at: new Date().toISOString() }, { telegram_user_id: `eq.${userId}` });
}

function getNetworks(userId: number) {
  return dbGet<Network>("tg_networks", { telegram_user_id: `eq.${userId}`, select: "*", order: "created_at.asc" });
}
...
function parseRouterDate(value: string) {
  const match = /^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\/(\d{2})\/(\d{4})\s+(\d{2}):(\d{2}):(\d{2})/i.exec(value);
  if (!match) return null;
  const months: Record<string, number> = { jan:0,feb:1,mar:2,apr:3,may:4,jun:5,jul:6,aug:7,sep:8,oct:9,nov:10,dec:11 };
  // Router timezone is Asia/Aden (+03:00) for PRONET and most target Yemen networks.
  return new Date(Date.UTC(Number(match[3]), months[match[1].toLowerCase()], Number(match[2]), Number(match[4]) - 3, Number(match[5]), Number(match[6])));
}
...
async function readStatus(network: Network) {
  ...
  try {
    ...
    return {
      ...
      ppp: ppp.find((item) => item.name === "pppoe-out1")?.running === "true" ? "🟢 متصل" : "⚪ غير متأكد",
    };
  } finally {
    client.close();
  }
...
async function readPing(network: Network) {
  ...
  try {
    ...
    const times = rows.map((row) => parseFloat(String(row.time || "").replace("ms", ""))).filter(Number.isFinite);
...
async function readVlan(network: Network, vlanId: number) {
  const client = clientFor(network);
  try {
    const vlans = await client.command("/interface/vlan/print", ["=.proplist=.id,name,vlan-id,disabled,rx-byte,tx-byte"]);
    ...
    const active = await client.command("/ip/hotspot/active/print", ["=.proplist=address"]);
    return {
      name: vlan.name || String(vlanId),
      rx,
      tx,
      total: rx + tx,
      online: active.filter((item) => String(item.address || "").startsWith(prefix)).length,
    };
  } finally {
    client.close();
  }
...
async function syncAndReadSales(network: Network): Promise<UMCard[]> {
  ...
  try {
    ...
    try {
      users = await client.command("/tool/user-manager/user/print", ["=.proplist=username,actual-profile,last-seen"]);
    } catch {
    ...
    for (const user of users) {
      ...
      try {
        sessions = await client.command("/tool/user-manager/session/print", ["=.proplist=user,from-time,user-ip", `?user=${username}`]);
      } catch {
        continue;
      }

      const ordered = sessions
        .map((session) => ({ session, date: parseRouterDate(session["from-time"] || "") }))
        .filter((item): item is { session: Record<string, string>; date: Date } => item.date instanceof Date)
...
async function processSetup(message: TgMessage, user: BotUser) {
  ...
  if (user.setup_state === "await_password") {
    ...
    try {
      ...
      const saved = await dbInsert<Network>("tg_networks", {
        telegram_user_id: user.telegram_user_id,
        label: temp.label,
        host: temp.host,
        port: temp.port,
...
async function runAction(chatId: number, user: BotUser, action: string) {
  ...
  if (action === "help") {
    return send(chatId, `${homeText()}\n\n<b>الأوامر:</b>\n/status\n/sales\n/ping\n/vlan 202\n/networks\n/add`, mainKeyboard);
  }
  if (action === "settings") return send(chatId, "⚙️ <b>الإعدادات</b>\nيمكنك إضافة أكثر من شبكة واختيار الشبكة النشطة من «شبكاتي».", mainKeyboard);
  if (action === "vlan_help") return send(chatId, "🧩 لمعرفة VLAN محدد أرسل مثلاً:\n<code>/vlan 202</code>");

  if (action === "networks") {
    const list = await getNetworks(user.telegram_user_id);
    if (!list.length) return send(chatId, "📭 لا توجد شبكات بعد.", mainKeyboard);
    const text = list.map((item, index) => `${index + 1}. ${item.id === user.active_network_id ? "🟢" : "⚪"} <b>${item.label}</b>\n   ${item.host}:${item.port}`).join("\n\n");
  ...
  if (action === "sales" && network) {
    try {
      ...
      const lines = sales.slice(0, 20).map((item, index) => `${index + 1}. 🎫 <code>${item.username}</code>${item.first_profile ? ` • ${item.first_profile}` : ""}${item.first_vlan_id ? ` • VLAN ${item.first_vlan_id}` : ""}`);
      return send(chatId, `💰 <b>مبيعات اليوم • أول دخول فقط</b>\n━━━━━━━━━━━━━━━━━━\n🆕 الكروت التي سجلت لأول مرة: <b>${sales.length}</b>\n\n${lines.join("\n") || "لا توجد مبيعات مسجلة حتى الآن."}${sales.length > 20 ? `\n\n… +${sales.length - 20} كرت` : ""}`);
    } catch (error) {
...
export async function handleTelegramUpdate(update: TgUpdate) {
  ...
  if (callback?.from) {
    ...
    if (action === "proto_api_ssl" || action === "proto_api") {
      ...
      const port = protocol === "api-ssl" ? 8729 : 8728;
      await setState(user.telegram_user_id, "await_username", { ...(user.setup_data || {}), protocol, port });
      return send(chatId, `👤 أرسل <b>API Username</b>.\n\nسيتم استخدام المنفذ <code>${port}</code>.`);
    }
  ...
  if (text.startsWith("/vlan ")) {
    ...
    if (!Number.isInteger(vlanId) || vlanId < 1 || vlanId > 4094) return send(message.chat.id, "⚠️ مثال صحيح: <code>/vlan 202</code>");
    const network = await getActiveNetwork(user);
    ...
    try {
      ...
      return send(message.chat.id, `🧩 <b>VLAN ${vlanId} • ${vlan.name}</b>\n━━━━━━━━━━━━━━━━━━\n📦 الاستهلاك: <b>${bytes(vlan.total)}</b>\n⬇️ Download: ${bytes(vlan.tx)}\n⬆️ Upload: ${bytes(vlan.rx)}\n👥 متصل الآن: <b>${vlan.online}</b>`);
    } catch (error) {
  ...
  if (text.startsWith("/use ")) {
    ...
    const list = await getNetworks(user.telegram_user_id);
    if (!Number.isInteger(index) || index < 1 || index > list.length) return send(message.chat.id, "⚠️ استخدم /networks ثم /use رقم الشبكة.");
    await dbPatch("tg_users", { active_network_id: list[index - 1].id, updated_at: new Date().toISOString() }, { telegram_user_id: `eq.${user.telegram_user_id}` });
    return send(message.chat.id, `✅ الشبكة النشطة الآن: <b>${list[index - 1].label}</b>`, mainKeyboard);
  }
