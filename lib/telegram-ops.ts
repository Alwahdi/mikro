...
type OpsKind="logs"|"interfaces"|"dhcp"|"hotspot"|"top_usage";
...
async function queueAgent(n:Network,uid:number,chatId:number,kind:OpsKind){
  const online=n.agent_last_seen_at?Date.now()-new Date(n.agent_last_seen_at).getTime()<60_000:false;
  const waiting=await send(chatId,`${online?"⏳":"🟠"} <b>${esc(n.identity||n.label)}</b>\n${online?"جاري تنفيذ القراءة عبر Agent…":"Agent غير ظاهر الآن؛ وضعت الطلب في الانتظار."}`);
  await dbInsert("tg_agent_commands",{network_id:n.id,telegram_user_id:uid,chat_id:chatId,reply_message_id:waiting.message_id,kind,payload:{},status:"pending"});
}

export async function handleTelegramOps(update:TgUpdate,kind:OpsKind){
  ...
  if(n.connection_mode==="agent"){await queueAgent(n,m.from.id,m.chat.id,kind);return true;}
  const c=client(n);
  try{
    if(kind==="logs"){
    ...
    if(kind==="hotspot"){
      // Same RouterOS socket: run sequentially, never Promise.all.
      const servers=await c.command("/ip/hotspot/print",["=.proplist=name,interface,disabled,profile"]);
      const active=await c.command("/ip/hotspot/active/print",["=.proplist=user,address,server,uptime"]);
      const users=await c.command("/ip/hotspot/user/print",["=.proplist=name,profile,disabled"]);
