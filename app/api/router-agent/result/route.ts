...
import { renderAgentDiagnosis } from "@/lib/telegram-diagnose";
...
function render(command: AgentCommand, data: Record<string, string>) {
  ...
  if (command.kind === "vlans") {
    ...
    const lines = rows.map((entry) => { const [id, name] = entry.split("@"); return `• <b>VLAN ${esc(id || "-")}</b> — ${esc(name || "-")}`; });
  ...
  if (command.kind === "sales") {
    const count = Number(data.count || 0), allCards = (data.cards || "").split(",").filter(Boolean), cards = allCards.slice(0, 30);
  ...
  if (command.kind === "card") {
    const totalDown = Number(data.download || 0), totalUp = Number(data.upload || 0), active = data.active === "1";
    const rows = (data.sessions || "").split(";").filter(Boolean).slice(0, 10);
    const lines = rows.map((entry, index) => { const [started, uptime, ip, cause] = entry.split("@"); return `${index + 1}. ${esc(started || "-")} • ⏱ ${esc(uptime || "-")}\n   IP ${esc(ip || "-")}${cause ? ` • ${esc(cause)}` : ""}`; });
...
async function persistAgentSales(networkId: string, data: Record<string, string>) {
  ...
  for (const username of cards) {
    const existing = await dbGet<{ username: string }>("tg_cards", { network_id: `eq.${networkId}`, username: `eq.${username}`, select: "username", limit: "1" });
    if (existing.length) continue;
    const now = new Date().toISOString();
    try { await dbInsert("tg_cards", { network_id: networkId, username, first_seen_at: now, first_seen_router_time: null, first_profile: null, first_vlan_id: null, last_seen_at: now, last_profile: null, last_vlan_id: null }); } catch {}
  }
}
...
export async function POST(req: NextRequest) {
  ...
  const data = parseBody(raw), commandId = data.id || "";
  if (!commandId) return new NextResponse("missing command id", { status: 400 });

  const commands = await dbGet<AgentCommand>("tg_agent_commands", { id: `eq.${commandId}`, network_id: `eq.${networkId}`, select: "id,network_id,chat_id,reply_message_id,kind,payload", limit: "1" });
  const command = commands[0];
  if (!command) return new NextResponse("unknown command", { status: 404 });
  const ok = data.status === "ok";
  await dbPatch("tg_agent_commands", { status: ok ? "success" : "error", result: data, error: ok ? null : data.error || "agent-command-failed", completed_at: new Date().toISOString() }, { id: `eq.${command.id}` });
  await markAgentSeen(networkId);

  if (command.kind === "status" && ok) {
    await dbPatch("tg_networks", { identity: data.identity || network.label, router_os_version: data.version || null, agent_capabilities: { transport: "https-fetch", status: true, ping: true, router: true, online: true, vlans: true, vlan: true, sales: true, card: true }, updated_at: new Date().toISOString() }, { id: `eq.${networkId}` });
  }
  if (command.kind === "sales") { try { await persistAgentSales(networkId, data); } catch (error) { console.error("Agent sales persistence failed", error); } }
  try { await deliver(command, render(command, data)); } catch (error) { console.error("Agent Telegram delivery failed", error); }
